...
if( isequal(dnn.type, 'BBRBM') || isequal(dnn.type, 'SRBM'))
    H = sigmoid( bsxfun(@plus, V * dnn.W, dnn.hidbias ) );
    
elseif(isequal(dnn.type, 'mix'))
    %v = bsxfun(@rdivide, V(:,1:dnn.dimV_con), dnn.sig );
    %V(:,1:dnn.dimV_con)=v;
    H = sigmoid( bsxfun(@plus, V * dnn.W, dnn.hidbias ) );
...
elseif( isequal(dnn.type, 'BBPRBM') )
    w2 = dnn.W .* dnn.W;
    pp = V .* ( 1-V );
    mu = bsxfun(@plus, V * dnn.W, dnn.hidbias );
    s2 = pp * w2;
